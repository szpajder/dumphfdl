...
static void rdkafka_delivery_report_cb(rd_kafka_t *rk, const rd_kafka_message_t *rkmessage, void *opaque) {
	UNUSED(rk);
...
static int out_rdkafka_init(void *selfptr) {
	...
	if (rdkafka_conf_set(conf, "bootstrap.servers", self->brokers) < 0) {
		return -1;
	}
	if (rdkafka_conf_set(conf, "acks", self->acks) < 0) {
		return -1;
	}
	...
	if (self->ssl_ca_location != NULL) {
		if (rdkafka_conf_set(conf, "ssl.ca.location", self->ssl_ca_location) < 0) {
			return -1;
		}
	}
	...
	if (self->sasl_username != NULL &&
		...
		self->security_protocol != NULL) {
			if (rdkafka_conf_set(conf, "sasl.mechanism", self->sasl_mechanism) < 0) {
				return -1;
			}
			if (rdkafka_conf_set(conf, "sasl.username", self->sasl_username) < 0) {
				return -1;
			}
			if (rdkafka_conf_set(conf, "sasl.password", self->sasl_password) < 0) {
				return -1;
			}
			if (rdkafka_conf_set(conf, "security.protocol", self->security_protocol) < 0) {
				return -1;
			}
	}
...
output_descriptor_t out_DEF_rdkafka = {
	.name = "rdkafka",
	.description = "Output to an Apache Kafka cluster",
